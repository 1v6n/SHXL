...
def step_impl_president_discards_card(context):
    ...
    context.chosen_policies, context.discarded_policy = context.game.presidential_policy_choice(
        context.president_received_cards
    )
...
def step_impl_chancellor_selects_to_enact(context):
    ...
    context.enacted_policy, context.chancellor_discarded = context.game.chancellor_policy_choice(
...
def step_impl_legislative_phase_completes(context):
    ...
    context.game.state.board.draw_policy = Mock(return_value=context.president_received_cards)

    if not hasattr(context, 'chosen_policies') or not hasattr(context, 'discarded_policy'):
    ...
    if not hasattr(context, 'enacted_policy') or not hasattr(context, 'chancellor_discarded'):
        context.enacted_policy = context.chosen_policies[0] if context.chosen_policies else TestPolicy("liberal")
        context.chancellor_discarded = context.chosen_policies[1] if len(context.chosen_policies) > 1 else TestPolicy("fascista")
    ...
    with patch('src.game.phases.election.ElectionPhase') as MockElectionPhase:
        ...
        try:
            ...
            print(f"Presidential choice mock: {context.game.presidential_policy_choice}")
...
def step_impl_remaining_cards_delivered_verification_robust(context, num_cards):
    ...
    if hasattr(context, 'cards_to_chancellor'):
        ...
        cards_found = context.president_received_cards[-num_cards:] if len(context.president_received_cards) >= num_cards else context.president_received_cards
...
def step_impl_legislative_phase_completes_successfully(context):
    ...
    if hasattr(context, 'policy_placed_in_tracker') and context.policy_placed_in_tracker:
...
def step_impl_bot_applies_discard_strategy(context):
    ...
    liberal_policies = [p for p in context.president_received_cards if p.policy_type == "liberal"]
    other_policies = [p for p in context.president_received_cards if p.policy_type != "liberal"]

    if other_policies:
        context.discarded_policy = other_policies[0]
        context.chosen_policies = [p for p in context.president_received_cards if p != context.discarded_policy][:2]
    else:
...
def step_impl_bot_chooses_via_strategy(context):
    ...
    liberal_policies = [p for p in context.chancellor_policies if p.policy_type == "liberal"]

    if liberal_policies:
        context.enacted_policy = liberal_policies[0]
        context.chancellor_discarded = [p for p in context.chancellor_policies if p != context.enacted_policy][0]
    else:
...
def step_impl_deck_has_less_than_cards(context, num_cards):
    ...
    context.game.state.board.draw_policy = Mock(side_effect=Exception("Insufficient cards"))
...
def step_impl_new_deck_formed(context):
    ...
    assert len(board.discard_pile) == 0, f"Discard pile should be empty after reshuffle, but has {len(board.discard_pile)} cards"

    if hasattr(board, 'policy_deck'):
        assert len(board.policy_deck) > 0, "Policy deck should have cards after reshuffle"
...
def step_impl_cards_presented_to_chancellor(context, num_cards):
    ...
    assert hasattr(context, 'cards_to_chancellor') or hasattr(context, 'chancellor_policies')
...
def step_impl_reshuffle_discard_pile(context):
    ...
    assert cards_to_reshuffle > 0, f"Need cards in discard pile for reshuffle, found {cards_to_reshuffle}"

    reshuffle_result = board.reshuffle()

    assert reshuffle_result == True, f"Reshuffle should succeed with {cards_to_reshuffle} cards"
...
def step_impl_bot_president_strategy_discard(context, num_cards):
    ...
    fascist_policies = [p for p in context.president_received_cards if p.policy_type == "fascista"]
...
def step_impl_card_must_be_among_received(context, num_cards):
    ...
    if hasattr(context, 'chancellor_selection_attempt') or hasattr(context, 'chancellor_policies'):
        if not hasattr(context, 'chancellor_policies'):
            context.chancellor_policies = create_test_policies()[:num_cards]
...
def step_impl_card_not_placed_in_tracker(context):
    ...
    if hasattr(context, 'enacted_policy') and context.enacted_policy.policy_type == "emergency":
        board = context.game.state.board
        assert board.liberal_policies == 0
        assert board.fascist_policies == 0
        assert board.communist_policies == 0
