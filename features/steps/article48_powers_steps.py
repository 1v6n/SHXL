...
def juego_con_3_jugadores(context):
    ...
    context.game.state.active_players = [
        context.presidente,
        context.canciller,
        context.otro_jugador,
    ]
    context.game.state.president.choose_revealer = Mock(
        return_value=context.otro_jugador
    )
...
def politica_removida_mazo(context):
    ...
    assert (
        politicas_actuales == context.politicas_originales - 1
...
def politica_permanece_mazo(context):
    ...
    politicas_actuales = len(context.game.state.board.policies)
    assert (
        politicas_actuales == context.politicas_originales
...
def sin_cambios_estado(context):
    ...
    assert (
        len(context.game.state.board.policies) == 0
...
def registra_sin_jugador_marcado(context):
    ...
    context.game.logger.log.assert_called_with(
        "No player is currently marked for execution."
    )
