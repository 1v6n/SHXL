...
def step_then_should_receive_instance(context, expected_class):
    ...
    assert expected_class_obj is not None, f"Clase no encontrada en el mapeo: {expected_class}"
    assert isinstance(context.power_result, expected_class_obj), \
...
def step_then_power_associated_with_game(context):
    ...
    assert context.power_result.game == context.game, \
...
def step_then_owner_should_be(context, expected_owner):
    ...
    assert context.power_owner == expected_owner_enum, \
...
def step_then_should_receive_valid_article48_power(context):
    ...
    assert context.article48_power in valid_article48_powers, \
...
def step_then_power_in_presidential_emergency_list(context):
    ...
    valid_powers = [
        "propaganda", "impeachment", "marked_for_execution",
        "policy_peek_emergency", "execution_emergency", "pardon"
    ]
...
def step_then_should_receive_valid_enabling_act_power(context):
    ...
    assert context.enabling_act_power in valid_enabling_act_powers, \
...
def step_then_power_in_chancellor_emergency_list(context):
    ...
    valid_powers = [
        "chancellor_propaganda", "chancellor_impeachment", "chancellor_marked_for_execution",
...
def step_then_all_fascist_powers_instantiable(context):
    ...
    for power_name in context.fascist_powers:
        try:
            ...
            assert power_instance is not None, f"No se pudo instanciar el poder: {power_name}"
        except Exception as e:
...
def step_then_each_power_has_valid_owner(context):
    ...
    for power_name in context.fascist_powers:
        ...
        assert isinstance(owner, PowerOwner), \
...
def step_then_all_should_return_specific_power(context, expected_power):
    ...
    if expected_power == "policy_peek_emergency":
        ...
        assert hasattr(context, 'multiple_article48_powers'), \
        ...
        assert hasattr(context, 'multiple_enabling_act_powers'), \
