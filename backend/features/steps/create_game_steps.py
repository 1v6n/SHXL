...
def _cleanup_local_processes():
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                if proc.info["cmdline"] and any(
                    "src.api.app" in str(cmd) for cmd in proc.info["cmdline"]
                ):
    ...
    try:
        result = subprocess.run(
            ["lsof", "-t", "-i:5000"], capture_output=True, text=True, timeout=5
        )
...
@when(
    'el cliente envía una petición POST a /newgame con 8 jugadores y estrategia "smart"'
)
def step_impl_post_newgame(context):
    payload = {"playerCount": 8, "strategy": "smart"}
    context.response = requests.post(f"{context.base_url}/newgame", json=payload)
